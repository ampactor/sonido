//! Graph topology DSL parser.
//!
//! Pure text → IR transformation with no audio dependencies.
//! Extends the pipe-chain syntax with `split()` for parallel routing:
//!
//! ```text
//! # Linear chain
...
use sonido_core::graph::MAX_SPLIT_TARGETS;
...
/// Errors from DSL parsing and validation.
...
impl<'a> Parser<'a> {
    ...
    fn parse_graph(&mut self) -> Result<GraphSpec, DslParseError> {
        let spec = self.parse_path()?;
        self.skip_ws();
        if let Some(ch) = self.peek() {
            return Err(DslParseError::UnexpectedChar { pos: self.pos, ch });
        }
        Ok(spec)
    }

    /// `path ::= segment ( '|' segment )*`
    fn parse_path(&mut self) -> Result<Vec<GraphNode>, DslParseError> {
    ...
    /// `segment ::= '-' | split_expr | effect_spec`
    fn parse_segment(&mut self) -> Result<GraphNode, DslParseError> {
        self.skip_ws();
    ...
    /// `split_expr ::= 'split(' path ( ';' path )+ ')'`
    fn parse_split(&mut self) -> Result<GraphNode, DslParseError> {
        ...
        loop {
            ...
            if self.peek() == Some(';') {
                ...
                if matches!(self.peek(), Some(';' | ')')) {
                    return Err(DslParseError::EmptySplitPath { pos: self.pos });
                }
        ...
        self.skip_ws();
        if self.peek() != Some(')') {
            return Err(DslParseError::UnclosedSplit { pos: open_pos });
        }
        self.advance(); // consume ')'

        if paths.len() < 2 {
            return Err(DslParseError::SplitTooFewPaths { count: paths.len() });
        }
        if paths.len() > MAX_SPLIT_TARGETS {
            return Err(DslParseError::SplitTooManyPaths {
                count: paths.len(),
    ...
    /// `effect_spec ::= name ( ':' key '=' value ( ',' key '=' value )* )?`
    fn parse_effect(&mut self) -> Result<GraphNode, DslParseError> {
        self.skip_ws();
        let start = self.pos;
        ...
        if name.is_empty() {
            return Err(DslParseError::EmptySplitPath { pos: start });
        }
    ...
    fn parse_params(&mut self, params: &mut HashMap<String, String>) -> Result<(), DslParseError> {
        loop {
            self.skip_ws();
            ...
            while let Some(ch) = self.peek() {
                ...
                if matches!(ch, '|' | ';' | ')' | ',') {
                    return Err(DslParseError::ParamError {
                        pos: key_start,
                        message: format!(
...
/// # Errors
///
/// Returns [`DslParseError`] on syntax errors (unclosed splits, missing `=`, etc.).
pub fn parse_graph_dsl(input: &str) -> Result<GraphSpec, DslParseError> {
    ...
    tracing::debug!("dsl_parse: {} nodes from spec", count_nodes(&spec));
    Ok(spec)
}
...
/// # Errors
///
/// Returns [`DslParseError::DryAtTopLevel`] if `-` appears outside a split.
pub fn validate_spec(spec: &GraphSpec) -> Result<(), DslParseError> {
    validate_nodes(spec, false)
}

fn validate_nodes(nodes: &[GraphNode], in_split: bool) -> Result<(), DslParseError> {
    for node in nodes {
        match node {
            GraphNode::Dry if !in_split => return Err(DslParseError::DryAtTopLevel),
            GraphNode::Dry => {}
            GraphNode::Effect { .. } => {}
            GraphNode::Split { paths } => {
                for path in paths {
                    if path.is_empty() {
                        return Err(DslParseError::EmptySplitPath { pos: 0 });
                    }
...
pub fn count_nodes(spec: &GraphSpec) -> usize {
    spec.iter()
        .map(|node| match node {
...
mod tests {
    ...
    fn validate_dry_at_top_level() {
        ...
        assert!(matches!(
            validate_spec(&spec),
            Err(DslParseError::DryAtTopLevel)
        ));
    }
    ...
    fn error_unclosed_split() {
        ...
        assert!(matches!(err, DslParseError::UnclosedSplit { .. }));
    }

    #[test]
    fn error_single_path_split() {
        let err = parse_graph_dsl("split(distortion)").unwrap_err();
        assert!(matches!(err, DslParseError::SplitTooFewPaths { count: 1 }));
    }

    #[test]
    fn error_empty_path_in_split() {
        let err = parse_graph_dsl("split(distortion; ; reverb)").unwrap_err();
        assert!(matches!(err, DslParseError::EmptySplitPath { .. }));
    }

    #[test]
    fn error_empty_input() {
        let err = parse_graph_dsl("").unwrap_err();
        assert!(matches!(err, DslParseError::ParamError { .. }));
    }
